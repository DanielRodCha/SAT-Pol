...
parseFProp xs = case (analiza formEqui xs) of
             [(n,[])]  -> n
             [(_,sal)] -> error ("entrada sin usar " ++ sal)
             []        -> error "entrada no valida"


formEqui :: Analizador FProp
formEqui = formImpl      >*> \i ->
           (simbolo "<->" >*> \_ ->
            formEqui      >*> \e ->
            resultado (Equi i e))
...
identificador' :: Analizador FProp
identificador' x = aux $ identificador x
                   where aux [(a,b)] = [(Atom a,b)]