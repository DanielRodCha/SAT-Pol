...
-- | __(tool (vvs,ps))__ is verified if the original set of formulas which
...
-- the set of variables which occurs in any polynomial from /ps/.

-- We should think if there exists any way to use the lazy power in the search
-- of zeros.

tool :: (S.Set (PolF2), S.Set (PolF2)) -> Bool
tool (vvs,ps) | S.null vvs        = S.notMember 0 ps
              | S.member 0 ps  = False
              | otherwise      = tool (vs,ps')
